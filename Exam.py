...
print("vous avez 8 tours pour trouver la solution d'un mot de 6 lettres,vous êtes au tour :",tour)

#Début de code
def tableaujoueur(tableau_mystere, tableau_joueur):
    for i in range (1,7):
        tableau_joueur[i-1] = mot_joueur
...
#def color (tableau_mystere ,tableau_joueur):
#	while tableau_joueur[1] == tableau_mystere[1]:
#		print(Force.RED + tableau_joueur[1])

mot_joueur = input("inscrivez votre mot :")
tableau_joueur = mot_joueur
#Fin de code
while tour < 8 :
    tour = tour + 1
#print(mot_joueur)
